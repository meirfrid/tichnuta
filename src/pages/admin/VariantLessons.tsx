...
const VariantLessons = () => {
  ...
  const handleSaveLesson = async () => {
    if (!lessonForm.title || !courseId || !variantId) {
  ...
  const handleMoveLesson = async (lessonId: string, direction: "up" | "down") => {
    ...
    [updatedLessons[currentIndex], updatedLessons[targetIndex]] = [
      updatedLessons[targetIndex], updatedLessons[currentIndex],
    ];
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle>ניהול שיעורים: {variant?.name}</CardTitle>
            <CardDescription>
              {course?.title} — הוסף, ערוך וסדר שיעורים למחזור לימוד זה
            </CardDescription>
          </CardHeader>
          <CardContent>
            {!showNewLessonForm && (
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      קישור ל-Google Drive, YouTube, Vimeo וכו'
                    </p>
                  </div>
                  <div>
                    ...
                    <TableRow key={lesson.id}>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          <Button size="sm" variant="ghost" onClick={() => handleMoveLesson(lesson.id, "up")} disabled={index === 0}>
                            <ChevronUp className="h-4 w-4" />
                          </Button>
                          <Button size="sm" variant="ghost" onClick={() => handleMoveLesson(lesson.id, "down")} disabled={index === lessons.length - 1}>
